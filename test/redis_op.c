...
int rop_redis_setvalue(redisContext *conn, const char *key, const char *value)
{
    int ret = 0;
    redisReply *reply = NULL;
...
int rop_redis_getvalue(redisContext *conn, const char *key, char *value)
{
    int ret = 0;
    redisReply *reply = NULL;
...
int rop_hash_getvalue(redisContext *conn, const char *key, const char *filed, char *value)
{
    int ret = 0;
    redisReply *reply = NULL;

	reply = redisCommand(conn, "HGET %s %s", key, filed);
    //rop_test_reply_type(reply);
    if (reply->type == REDIS_REPLY_ERROR || reply->type == REDIS_REPLY_NIL) {
        printf("redisCommand error \n");
        ret = -1;
...
int rop_hash_setvalue(redisContext *conn, const char *key, const char *filed, char *value)
{
    int ret = 0;
    redisReply *reply = NULL;

	reply = redisCommand(conn, "HSET %s %s %s", key, filed, value);
    //rop_test_reply_type(reply);
    if (reply->type == REDIS_REPLY_ERROR || reply->type == REDIS_REPLY_NIL) {
        printf("redisCommand error \n");
        ret = -1;
...
int rop_hash_getall(redisContext *conn, const char *key,char (*value)[128], int *len)
{
    int ret = 0;
    int i = 0;
    redisReply *reply = NULL;

	reply = redisCommand(conn, "HGETALL %s", key);
    //rop_test_reply_type(reply);
    if (reply->type != REDIS_REPLY_ARRAY) {
        printf("redisCommand error \n");
        ret = -1;
    ...
    for (i = 0; i < reply->elements; ++i) {
        strcpy(value[i], reply->element[i]->str);
    }
    *len = reply->elements;
END:
	freeReplyObject(reply);
